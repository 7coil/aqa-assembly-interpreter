...
const instructions = [
  ...
  new Instruction('Raise to power', 'POW <register>, <register>, <operand>', (memory, registers, labels, data) => {
    // If the operand is a register, use the register.
    if (data[3] === 'R') {
      registers[parseInt(data[1], 10)] = registers[parseInt(data[2], 10)] ** registers[parseInt(data[4], 10)];
    } else {
...
const parse = (input) => {
  ...
  const program = lines
    ...
    .map((line) => {
      return {
        line,
        operation: instructions.find(set => set.regexp.test(line))
      }
    });
